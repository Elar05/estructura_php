  ...
  <div class="pd-20 card-box mb-30">
    <div class="clearfix">
      ...
      <div class="pull-right">
        <a href="<?= URL ?>/user-action" class="btn btn-primary btn-sm"><i class="fa fa-arrow-left"></i> Back</a>
      </div>
    </div>
    <form action="<?= URL . $this->d['action'] ?>" method="post">
      <input type="hidden" name="id" value="<?= $this->d['userAction']['id'] ?? '' ?>">
      <?php $this->showMessages() ?>
      <div class="form-group">
        <label for="name">Name Action</label>
        <input class="form-control" type="text" name="name" id="name" placeholder="Johnny Brown" value="<?= $this->d['userAction']['name'] ?? '' ?>">
      </div>
